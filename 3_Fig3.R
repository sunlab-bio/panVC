...
av_expr_v11 <- AverageExpression(
  seurat_obj[,seurat_obj$cell_type== 'Ven.c11.TMSB4X'], assays = "RNA",
  group.by = "disease" )[["RNA"]] %>% 
  as.data.frame()
...
annotation_col <-  data.frame(Disease = factor(colnames(expr)), Celltype = c(rep('Ven_c10_VCAM1', 9), rep('Ven_c11_TMSB4X', 9)))
colnames(expr) <- c(1:18)

rownames(annotation_col) <- c(1:18)
ann_colors = list(
  Celltype = c(Ven_c10_VCAM1 = "#6a73cf", Ven_c11_TMSB4X = "#916CB2"),
