...
seurat_obj$type_per <- factor(seurat_obj$type_per, 
                              levels = c('Art.c01.DKK2 (3.51%)','Art.c02.NEBL (3.44%)','Art.c03.VEGFA (8.01%)','Cap.c04.PAPSS2 (12.96%)', 
                                         ...
                                         'Ven.c10.VCAM1 (3.87%)','Ven.c11.TMSB4X (1.90%)','LEC.c12.FLT4 (0.55%)','Endo.c13.NPR3 (0.99%)'))
...
DotPlot(seurat_obj, group.by = 'cell_type',
        features= rev(c('DKK2', 'NEBL', 'VEGFA', 
                        'PAPSS2', 'RGCC','RDH10', 'CA2', 'TMEM163', 'RGS6', 'VCAM1', 'TMSB4X', 
                        'FLT4', 'NPR3')))+
...
library(ggplot2)
case <- 'F1_'
seurat <- qread('~/project/PH/filter_name/results/SCT15_1_P2/files/named_new/Seurat_202504.qs')
shannon_equitability <- function(freq_vec) {
  M <- length(freq_vec)
  freq_vec_filtered <- freq_vec[freq_vec > 1e-10]  # 避免log(0)
  H <- -sum(freq_vec_filtered * log2(freq_vec_filtered))
  E_H <- H / log2(M)
  E_H <- ifelse(is.na(E_H) | is.infinite(E_H), 0, E_H)
  return(E_H)
}
count_by_sample <- seurat@meta.data %>%
  group_by(sample, group, cell_type) %>%
  summarise(n = n(), .groups = "drop")

sample_celltype_count <- count_by_sample %>%
  group_by(sample, group) %>%
  summarise(
    cell_type_num = n_distinct(cell_type), 
    total_cell = sum(n),                  
    .groups = "drop"
  )
valid_samples <- sample_celltype_count %>%
  dplyr::filter(
    cell_type_num >= 2,   
    total_cell >= 10         
  ) %>%
...
count_by_sample_filtered <- count_by_sample %>%
  dplyr::filter(sample %in% valid_samples)

freq_by_sample <- count_by_sample_filtered %>%
  group_by(sample, group) %>%
  mutate(freq = n / sum(n)) %>%
  ungroup()

sample_diversity <- freq_by_sample %>%
  group_by(sample, group) %>%
  summarise(
    diversity = shannon_equitability(freq),
    .groups = "drop"
  )
print(sample_diversity)

group_diversity <- sample_diversity %>%
  group_by(group) %>%
  summarise(
    mean_diversity = round(mean(diversity), 3),
    sd_diversity = round(sd(diversity), 3),
    n = n(),
    se_diversity = round(sd_diversity / sqrt(n), 3),
    .groups = "drop"
  )
...
prop_df <- meta %>%
  ...
  ungroup() %>%
  dplyr::select(group, cell_type, mean_freq)
prop_df$group <- factor(prop_df$group, levels = c("Health", "CVD"))
ggplot(prop_df, aes(x = cell_type, y = mean_freq, fill = group)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(
    data = group_diversity,
    aes(x = Inf, y = Inf, label = paste0("diversity: ", mean_diversity )),
    hjust = 1.1, vjust = 1.1, size = 3.5) +
  facet_wrap(~group, ncol = 1, scales = "free_x") +
  scale_fill_manual(values = c("Health" = "#B3DE69", "CVD" = "#b55f31")) +
  labs(x = NULL, y = "Frequency", fill = "Group") +
  theme_bw() +
...
ggsave(file.path(outdir2, paste0(case, "propor_barplot.pdf")),
       ggplot2::last_plot(),width = 4,height = 4.5)
write.table(prop_df, file.path(outdir, paste0(case, "propor_barplot.csv")),
...
data <- data[c('Art.c01.DKK2', 'Art.c02.NEBL', 'Art.c03.VEGFA', 'Cap.c04.PAPSS2', 'Cap.c05.RGCC', 
               ...
               'Ven.c11.TMSB4X', 'LEC.c12.FLT4' , 'Endo.c13.NPR3'), ]
